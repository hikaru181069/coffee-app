...
describe("GET /api/discover/all（Discover専用ページ用の全産地一覧）", () => {
  test("トークン無しは401", async () => {
    const res = await request(app).get(`${DISCOVER_PATH}/all`);

    expect(res.status).toBe(401);
    expect(res.body.error.code).toBe("UNAUTHORIZED");
  });

  test("条件を満たす産地が無ければ空配列を返す", async () => {
    const res = await request(app).get(`${DISCOVER_PATH}/all`).set("Authorization", alice.authHeader);

    expect(res.status).toBe(200);
    expect(res.body).toEqual({ data: { origins: [] } });
  });

  test("自分の記録だけから、条件を満たす産地すべてを返す（他ユーザーの記録は混ざらない）", async () => {
    await seedTestMasterData();
    const ethiopia = await Origin.findOne({ normalizedName: "ethiopia" });
    ...
    await createRecordFor(alice.user._id, { originId: ethiopia._id, processId: natural._id });
    await createRecordFor(alice.user._id, { originId: ethiopia._id, processId: natural._id });
    await createRecordFor(alice.user._id, { originId: kenya._id, processId: natural._id });
    await createRecordFor(alice.user._id, { originId: kenya._id, processId: natural._id });
    ...
    expect(res.status).toBe(200);
    const labels = res.body.data.origins.map((group) => group.originLabel);
    expect(labels.sort()).toEqual(["Ethiopia", "Kenya"]);
  });
});
