...
- **ホバー時のプレビュー（`GraphCommunities.jsx`、GraphLegendの直下）**:
  キャンバス上でノードにカーソルを重ねている間だけ、そのノードが属する
  グループ（記録件数・代表属性のチップ）を表示する。既存の「ホバーで
  隣接ノードのラベルを出す」挙動（`GraphCanvas.jsx`）と同じ操作感に
  揃えた。何もホバーしていない・該当グループが無いときは何も表示しない
  （静かな道具の方針）。PCでの補助的な導線のため、モバイル（ホバー操作が
  無い環境）では機能しない
