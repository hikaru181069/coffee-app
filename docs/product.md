...
MVPの段階ではAI推薦機能は実装しない。ただし、固定辞書によるルールベースの
キーワード検出（例: notesの自由記述から「甘い」「苦い」といった味覚語を
部分文字列一致で抽出し、知識グラフのノードにする機能）は許容する。
機械学習モデルや自然言語処理ライブラリを使わず、条件分岐と静的な辞書
だけで完結するため。AI/NLPによる自由な意味解析・感情分析・要約・
推薦文生成は引き続きスコープ外。
