...
（記録カードのライブプレビュー案は、検討の結果見送った）。第1弾
（選択肢への視覚言語反映・保存の瞬間の演出）に続き、第2弾でレシート/
チケット風の単一カードへ全面再構成し、保存後に「発見」を見せる画面を
追加した（`RecordForm.jsx`）。

常時表示のゾーン（破線の区切り線で分ける。新しい色トークンは追加せず、
既存の`line/60`のまま）:

- **HEADER**: title・consumedAt（`font-mono`）・recordType・cafeName（カフェ記録のみ）
- **TASTE**: 味覚グラフ（甘み・苦み・酸味・コク・香り・後味、それぞれ
  1〜5、任意）を`TasteSliderInput`（スライダー、値は`RatingInput`と同じ
  1〜5の文字列）で入力する。以前はCoffee Detailsの折りたたみの中に
  あったが、味覚評価はこのアプリの中心体験の1つであるため、発見しやすさを
  優先してCoffee Detailsから独立させ、常時表示のゾーンにした
- **NOTES**: notes
- **FOOTER**: 総合評価（★、`RatingInput`）+ 保存ボタン

Coffee Detailsとして段階的に見せるゾーン（Record First、初期状態は
閉じている）:
...
- **選択肢への知識グラフの視覚言語の反映**: 産地・農園・品種・精製方法・
  焙煎度・フレーバーの各`FormField`のlabelに、知識グラフのノード種別
  アイコン（`features/graph/utils/nodeVisuals.js`の`getNodeVisual(type)`、
  `AttributeLabel.jsx`）を添え、「選ぶ」段階の見た目を記録詳細・グラフで
  見る見た目と地続きにする。産地だけは、記録カード・グラフ・世界地図で
  既に使っている産地ごとのアクセントカラー（`utils/originAccent.js`）を
  選択肢自体にも反映するため、ネイティブ`<select>`をやめ専用の
  `OriginPicker.jsx`（単一選択のチップ、色ドット付き）に置き換えた。
  ネイティブ`<option>`には信頼できる背景色スタイリング手段が無いための
  対応。精製方法・焙煎度は`nodeVisuals.js`上も値ごとの色を持たない
  （種別1つにつき1色）ため、ネイティブ`<select>`のまま変更していない。
- **保存の瞬間の演出**: 保存ボタン押下後、ローディングスピナー
  （`Loader2`）が一瞬チェックマーク（`Check`）に切り替わり、ラベルも
  「保存中...」→「保存しました」に変わってから記録詳細ページへ遷移する
  （`RecordFormPage.jsx`の`isJustSaved`、約450ms）。ボタンの色・形
  （`primaryButtonClass`）は変えず、アイコンとラベルの切り替えのみに
  留めている（「静かな道具」という世界観、過去にHome画面のCTAで
  「派手すぎる」という指摘を受けた経緯を踏まえた）。
  `prefers-reduced-motion`が有効な環境では演出をスキップし、従来どおり
  即座に遷移する。

**保存後の発見**（第2弾で追加）: 記録の保存によって新しく生まれた
つながり・達成したマイルストーン（`backend/core/discoveries/
discoveryBuilder.js`。ある属性ノードの初登場、または2/3/5/10件目への
到達）が1件以上あるとき、詳細ページへ遷移する前に専用のインタース
ティシャル画面（`SaveDiscoveryReveal.jsx`）を挟む。記録のタイトル・
評価（★）と、発見ごとに知識グラフの種別アイコン・色（`getNodeVisual`）
+文言+「つながりを見る」（該当エンティティ詳細ページへのリンク）を
並べ、「記録を見る」ボタンで詳細ページへ進む。バッジ・紙吹雪・効果音
などのゲーミフィケーション演出は入れず、テキストと既存の視覚言語だけで
構成する（保存の瞬間の演出と同じ「静かな道具」の方針）。発見が無い保存
では、従来どおり保存ボタンのチェックマーク演出→即座に詳細ページへ遷移
する。記録の編集（PATCH）では発見演出は出さない（「新しい体験を記録
した」文脈ではないため）。判定はAI/NLPを使わず、既存の閾値判定パターン
（Insight・Discover・Similar Recordsと同じ）を踏襲している
（docs/features.md「Save Discoveries」参照）。
