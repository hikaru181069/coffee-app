...
2026-09、「Statsのようにダッシュボード化すべきか」という相談を受けた。
一本道の設定フォーム（並べて見せる数値・ランキングが無い）という性質上、
KPIストリップやグリッドは合わないと判断し、ダッシュボード化はしないと
結論した。代わりに「余白・タイポを他の詳細系ページと揃えて」という
指摘を受け、次の2点を修正した:

- 各section見出し（表示言語・アカウント情報・パスワードを変更・
  アカウントを削除）を、RecordDetailPage/EntityDetailPage/StatsPageと
  同じ`text-base`（5段階タイプスケールの「カード内タイトル」相当）へ
  統一した（以前は`text-sm`のままだった）
- 見出し→本文の余白を、他ページと同じ`mt-5`へ統一した（以前は`mt-4`）
- 名前変更のsectionだけ他の3sectionと違い見出しが無かったため、
  `profile.accountHeading`（「アカウント情報」。メールアドレスの
  読み取り専用表示もこのsection内にあるため「名前」ではなくこの
  文言にした）を新設して揃えた
